...
//		}
//	}
	public void addNodeRecursively(int value) {
		root=addNodeRec(value,root);
	}
	
	public Node addNodeRec(int value,Node root) {
		if(root==null) {
			root=new Node(value);
			return root;
		}
		if(value<root.data) {
			root.left=addNodeRec(value, root.left);
		}else if(value>root.data) {
			root.right=addNodeRec(value, root.right);
		}
		return root;
	}
